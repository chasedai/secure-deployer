...
export default function TerminalPage() {
  ...
  useEffect(() => { getConfig().then((c) => { if (!cwd) setCwd(c.defaultCwd); }).catch(() => {}); }, []);
  ...
  const runCommand = async () => {
    ...
    try {
      const res = await fetch("/dash/terminal/exec", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ cmd: trimmed, cwd: cwd || undefined }),
      });
      if (res.headers.get("content-type")?.includes("text/event-stream")) {
        ...
        if (data.error) setOutput((o) => [...o, { type: "error", text: data.error }]);
      }
  ...
  return (
    <div className="p-8 h-full flex flex-col">
      ...
      <div className="mb-3 flex gap-2 items-center">
        <span className="text-sm font-medium" style={{ color: "var(--text-secondary)" }}>{t("terminal.cwd")}</span>
        <input value={cwd} onChange={(e) => setCwd(e.target.value)} className="flex-1 text-sm font-mono" placeholder="/home/ubuntu" />
      </div>
      <div className="flex-1 min-h-0 rounded-2xl overflow-hidden flex flex-col" style={{ background: "var(--bg-card)", boxShadow: "var(--shadow-sm)", border: "1px solid var(--border-light)" }}>
        <div ref={outputRef} className="flex-1 overflow-y-auto p-4 font-mono text-sm leading-6" style={{ background: "#1a1a1a" }} onClick={() => inputRef.current?.focus()}>
          {output.map((line, i) => <div key={i} className="whitespace-pre-wrap break-all" style={lineColor(line.type)}>{line.text}</div>)}
          {running && <div className="flex items-center gap-2 mt-1" style={{ color: "#0071e3" }}><Loader2 size={14} className="animate-spin" /> {t("terminal.running")}</div>}
        </div>
        <div className="flex items-center" style={{ background: "#1a1a1a", borderTop: "1px solid #333" }}>
          <span className="pl-4 font-mono text-sm" style={{ color: "#34c759" }}>$</span>
          <input ref={inputRef} value={cmd} onChange={(e) => setCmd(e.target.value)} onKeyDown={handleKeyDown} placeholder={t("terminal.placeholder")} disabled={running} className="terminal-input flex-1 font-mono text-sm" autoFocus />
          <button onClick={runCommand} disabled={running || !cmd.trim()} className="px-4 py-3 disabled:opacity-30 cursor-pointer" style={{ color: "#34c759" }}><Play size={16} /></button>
        </div>
      </div>
    </div>
  );
